  ...
  *)
  ...
  | Map (FunName farg, arr_exp, elem_type, ret_type, pos) =>
    let val addr_reg = newName "addr_reg"
        ...
        val loop_map = applyRegs(farg, [addr_reg], place, pos)
      
        val loop_footer = [ Mips.ADDI (addr_reg, addr_reg, "increment")
                            , Mips.ADDI (result_reg, place, "increment")
                            , Mips.ADDI (i_reg, i_reg, "1")
                            , Mips.J loop_beg
                            , Mips.LABEL loop_end
                            ]
